...
function autoReadLastAssistant(){
  ...
  const clean=_stripForTTS(text);
  if(!clean) return;
  // Chatterbox TTS via the fork's shared streaming helper (chunked; defined in
  // _HERMES_TTS_JS). Auto-read previously sent the whole response as ONE /api/tts
  // request, which blew past the server timeout on long turns (Chatterbox ~70ms/char).
  // Preferred when wired; falls through to the stock engine dispatch when absent.
