...
function attachLiveStream(activeSid, streamId, uploaded=[], options={}){
  ...
  function _wireSSE(source){
    ...
    source.addEventListener('token',e=>{
      ...
      // Exception: TTS auto-read (#499) speaks complete sentences mid-generation and
      // needs the live visible text — pay for the parse only while it will consume it.
      let _arWants=false;
      try{ _arWants=!!(window._hermesAutoRead&&window._hermesAutoRead.wants&&window._hermesAutoRead.wants()); }catch(_){}
      if(assistantRow&&!_arWants){
        ensureAssistantRow();
        _scheduleRender();
      }else{
        const parsed=_parseStreamState();
        try{ if(_arWants) window._hermesAutoRead.feed((parsed&&parsed.displayText)||''); }catch(_){}
        if(assistantRow||String((parsed&&parsed.displayText)||'').trim()) ensureAssistantRow();
